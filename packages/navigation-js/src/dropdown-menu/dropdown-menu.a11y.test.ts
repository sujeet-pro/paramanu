...
function createDropdownHTML(open = false): string {
  const rootClasses = dropdownClasses({ open })
  return `
...
describe("dropdown menu accessibility", () => {
  it("trigger has aria-haspopup", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createDropdownHTML()}</body>`)
    const trigger = dom.window.document.querySelector("button")
    expect(trigger?.getAttribute("aria-haspopup")).toBe("true")
  })

  it("trigger has aria-expanded=false when closed", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createDropdownHTML(false)}</body>`)
    const trigger = dom.window.document.querySelector("button")
    expect(trigger?.getAttribute("aria-expanded")).toBe("false")
  })

  it("trigger has aria-expanded=true when open", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createDropdownHTML(true)}</body>`)
    const trigger = dom.window.document.querySelector("button")
