...
function createCloseButtonHTML(
  options: Parameters<typeof closeButtonClasses>[0] = {},
  attrs: string = "",
): string {
  const classes = closeButtonClasses(options)
...
describe("close button accessibility", () => {
  it("renders as a focusable button element", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createCloseButtonHTML()}</body>`)
  ...
  it("disabled button has aria-disabled", () => {
    const dom = new JSDOM(
      `<!DOCTYPE html><body>${createCloseButtonHTML({ disabled: true })}</body>`,
    )
    const button = dom.window.document.querySelector("button")
  ...
  it("supports custom aria-label", () => {
    const html = createCloseButtonHTML({}, 'aria-label="Close dialog"')
