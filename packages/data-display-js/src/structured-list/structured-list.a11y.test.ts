...
function createStructListHTML(options: StructListClassesOptions = {}): string {
  const classes = structListClasses(options)
...
describe("structured list accessibility", () => {
  it("has role=table on root element", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createStructListHTML()}</body>`)
