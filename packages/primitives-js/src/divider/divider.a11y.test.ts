...
function createHorizontalDividerHTML(options: Parameters<typeof dividerClasses>[0] = {}): string {
  const classes = dividerClasses({ ...options, orientation: "horizontal" })
  return `<hr class="${classes}" aria-orientation="horizontal" />`
}

function createVerticalDividerHTML(options: Parameters<typeof dividerClasses>[0] = {}): string {
...
describe("divider accessibility", () => {
  it("horizontal divider uses semantic hr element", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createHorizontalDividerHTML()}</body>`)
  ...
  it("vertical divider uses div with role=separator", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createVerticalDividerHTML()}</body>`)
