...
function createNProgressHTML(value: number = 50, options: NProgressClassesOptions = {}): string {
...
describe("nprogress accessibility", () => {
  it("has role=progressbar", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createNProgressHTML()}</body>`)
    const el = dom.window.document.querySelector(".pm-nprogress")
    expect(el?.getAttribute("role")).toBe("progressbar")
  })

  it("has aria-valuenow, aria-valuemin, and aria-valuemax", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createNProgressHTML(50)}</body>`)
    const el = dom.window.document.querySelector(".pm-nprogress")
