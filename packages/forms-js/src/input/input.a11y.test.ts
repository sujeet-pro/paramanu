...
describe("input accessibility", () => {
  ...
  it("disabled input has disabled attribute and aria-disabled", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createInputHTML({ disabled: true })}</body>`)
    const input = dom.window.document.querySelector("input")
  ...
  it("invalid input has aria-invalid", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createInputHTML({ invalid: true })}</body>`)
    const input = dom.window.document.querySelector("input")
    expect(input?.getAttribute("aria-invalid")).toBe("true")
  })

  it("read-only input has readonly attribute", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createInputHTML({ readOnly: true })}</body>`)
    const input = dom.window.document.querySelector("input")
