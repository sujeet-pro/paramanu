...
function createPasswordInputHTML(
  options: Parameters<typeof passwordInputClasses>[0] = {},
  attrs: string = "",
): string {
  const wrapperClasses = passwordInputClasses(options)
  ...
  return `<div class="${wrapperClasses}">
    ...
    <button class="pm-password-input__toggle" type="button" aria-label="Toggle password visibility">Show</button>
  </div>`
}

describe("password input accessibility", () => {
  it("renders with a wrapper div and input element", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createPasswordInputHTML()}</body>`)
    const wrapper = dom.window.document.querySelector(".pm-password-input")
  ...
  it("toggle button has aria-label", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createPasswordInputHTML()}</body>`)
    const toggle = dom.window.document.querySelector(".pm-password-input__toggle")
  ...
  it("disabled input has disabled attribute and aria-disabled", () => {
    const dom = new JSDOM(
      `<!DOCTYPE html><body>${createPasswordInputHTML({ disabled: true })}</body>`,
    )
    const input = dom.window.document.querySelector("input")
  ...
  it("invalid input has aria-invalid", () => {
    const dom = new JSDOM(
      `<!DOCTYPE html><body>${createPasswordInputHTML({ invalid: true })}</body>`,
    )
    const input = dom.window.document.querySelector("input")
  ...
  it("supports aria-label on input", () => {
    const dom = new JSDOM(
      `<!DOCTYPE html><body>${createPasswordInputHTML({}, 'aria-label="Password"')}</body>`,
    )
    const input = dom.window.document.querySelector("input")
