...
function createTimepickerHTML(options: Parameters<typeof timepickerClasses>[0] = {}): string {
  ...
  return `<div class="${classes}"><input class="pm-timepicker__input" type="text" role="combobox" ${expandedAttr} aria-haspopup="listbox"${disabledAttr}${invalidAttr} aria-label="Select time" /><div class="pm-timepicker__dropdown" role="listbox"><div class="pm-timepicker__column" role="group" aria-label="Hours"><button class="pm-timepicker__option" role="option" aria-selected="false">01</button><button class="pm-timepicker__option pm-timepicker__option--selected" role="option" aria-selected="true">02</button></div><div class="pm-timepicker__column" role="group" aria-label="Minutes"><button class="pm-timepicker__option" role="option" aria-selected="false">00</button><button class="pm-timepicker__option" role="option" aria-selected="false">15</button></div></div></div>`
}
...
describe("time picker accessibility", () => {
  ...
  it("dropdown has role listbox", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createTimepickerHTML()}</body>`)
    const dropdown = dom.window.document.querySelector(".pm-timepicker__dropdown")
  ...
  it("selected option has aria-selected=true", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createTimepickerHTML()}</body>`)
    const selected = dom.window.document.querySelector(".pm-timepicker__option--selected")
  ...
  it("options are button elements", () => {
    const dom = new JSDOM(`<!DOCTYPE html><body>${createTimepickerHTML()}</body>`)
    const option = dom.window.document.querySelector(".pm-timepicker__option")
